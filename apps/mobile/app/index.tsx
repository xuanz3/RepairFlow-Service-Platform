    ...
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      <View style={styles.headerRow}>
        <View>
          ...
          <Text style={styles.subtitle}>Capture devices and evidence even when the network is unavailable.</Text>
        </View>
        <View style={styles.offlineBadge}><Text style={styles.offlineText}>Offline ready</Text></View>
      </View>
      ...
      <View style={styles.actionCard}>
        <View>
          ...
          <Text style={styles.cardCopy}>Customer, device condition, reported fault and intake evidence.</Text>
        </View>
        <View style={styles.actionButton}><Text style={styles.actionButtonText}>＋</Text></View>
      </View>
      ...
      {queueItems.map((item) => (
        <View style={styles.queueCard} key={item.reference}>
          <View style={styles.deviceMark}><Text style={styles.deviceMarkText}>{item.category[0]}</Text></View>
          <View style={styles.queueBody}>
            <Text style={styles.queueTitle}>{item.manufacturer} {item.model}</Text>
            <Text style={styles.queueMeta}>{item.reference} · {item.status}</Text>
          </View>
      ...
      <View style={styles.syncCard}>
        ...
        <View>
          ...
          <Text style={styles.syncCopy}>No pending changes. SQLite and secure storage boundaries are configured.</Text>
        </View>
      </View>
    </ScrollView>
  );
...
const styles = StyleSheet.create({
  ...
  offlineBadge: { alignSelf: 'flex-start', borderRadius: 999, backgroundColor: '#1f3a35', paddingHorizontal: 10, paddingVertical: 7 },
  offlineText: { color: colour.success, fontSize: 11, fontWeight: '800' },
  actionCard: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: spacing.lg, padding: spacing.lg, borderRadius: radius.lg, backgroundColor: '#252d4c' },
  ...
  actionButton: { width: 44, height: 44, borderRadius: 15, alignItems: 'center', justifyContent: 'center', backgroundColor: colour.accent },
  actionButtonText: { color: 'white', fontSize: 26, lineHeight: 28 },
  sectionHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: spacing.lg, marginBottom: 2 },
  sectionTitle: { color: colour.text, fontSize: 16, fontWeight: '800' },
  sectionMeta: { color: colour.textMuted, fontSize: 12 },
  queueCard: { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 14, borderWidth: 1, borderColor: colour.border, borderRadius: radius.md, backgroundColor: colour.surface },
  deviceMark: { width: 40, height: 40, alignItems: 'center', justifyContent: 'center', borderRadius: 12, backgroundColor: colour.surfaceRaised },
  ...
  syncCard: { flexDirection: 'row', gap: 12, marginTop: spacing.lg, padding: 14, borderRadius: radius.md, backgroundColor: '#16241f' },
